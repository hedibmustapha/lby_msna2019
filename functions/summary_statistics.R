...
percent_with_confints_select_one <-
  function(dependent.var,
    ...
    sanitised<-datasanitation_design(design,dependent.var,independent.var = NULL,
                                     hypegrammaR:::datasanitation_summary_statistics_percent_with_confints_select_one)

    if(!sanitised$success){
      warning(sanitised$message)
      return(hypegrammaR:::datasanitation_return_empty_table(data = design$variables, dependent.var, independent.var))}
    ...
    tryCatch(
      expr = {
        result_hg_format <-
        {
          ...
          result_svy_format <-
            svymean(formula(paste0("~", dependent.var)), design, level = 0.95,na.rm = TRUE) %>% cbind(., confint(.))
...
percent_with_confints_select_multiple <- function(dependent.var,
  ...
  hypegrammaR:::question_matches_choices(design$variables, dependent.var, sm.columns = dependent.var.sm.cols)
  ...
  sapply(dependent.var.sm.cols, function(x){
    ...
    sanitised<-datasanitation_design(design,dependent.var,independent.var = NULL,
                                     hypegrammaR:::datasanitation_summary_statistics_percent_sm_choice)
    if(!sanitised$success){
      warning(sanitised$message)
      return(hypegrammaR:::datasanitation_return_empty_table(data = design$variables, dependent.var))
      }
    design<-sanitised$design
    }
    )
  ...
  result_hg_format <- lapply(names(choices), function(x) {
    ...
    result_svy_format <-
      svymean(formula(paste0("~", x)), design, level = 0.95,na.rm = TRUE) %>% cbind(., confint(.))
    result_svy_format <-
...
percent_with_confints_select_one_groups <- function(dependent.var,
  ...
  sanitised<-datasanitation_design(design,dependent.var,independent.var,
                                   hypegrammaR:::datasanitation_summary_statistics_percent_groups)
  if(!sanitised$success){
    warning(sanitised$message)
    return(hypegrammaR:::datasanitation_return_empty_table(data = design$variables, dependent.var,independent.var))}
...
percent_with_confints_select_multiple_groups <-
  function(dependent.var,
    ...
    sapply(dependent.var.sm.cols, function(x){
      ...
      dependent.var <- names(design$variables)[x]
      sanitised<-datasanitation_design(design,dependent.var,independent.var,
                                       hypegrammaR:::datasanitation_summary_statistics_percent_sm_choice_groups)
      if(!sanitised$success){
        warning(sanitised$message)
        return(hypegrammaR:::datasanitation_return_empty_table_NA(data = design$variables, dependent.var, independent.var)) ### hack because in the etch case of a numeric dependent the whole thing goes
      }
      design<-sanitised$design
    })
...
mean_with_confints <- function(dependent.var,
  ...
  sanitised<-datasanitation_design(design,dependent.var,independent.var = NULL,
                                   hypegrammaR:::datasanitation_summary_statistics_mean)
  if(!sanitised$success){
    warning(sanitised$message)
    return(hypegrammaR:::datasanitation_return_empty_table(design$variables, dependent.var))}
...
mean_with_confints_groups <- function(dependent.var,
  ...
  results <- unique.independent.var.values %>%
    lapply(function(x) {
      ...
      colnames(dependent_value_x_stats) <-
        c("independent.var.value", "numbers", "min", "max")
      se = (dependent_value_x_stats[2]-dependent_value_x_stats[3])/1.96
      colnames(se) <- "se"
      data.frame(
        dependent.var = dependent.var,
        independent.var = independent.var,
