...
percent_with_confints_select_one <-
  function(dependent.var,
    ...
    if(!sanitised$success){
      ...
      return(hypegrammaR:::datasanitation_return_empty_table(data = design$variables, dependent.var, independent.var,message = sanitised$message))}
    ...
    tryCatch(
      expr = {
        ...
        get_confints<-purrr::possibly(function(...){
          ...
          confints<-survey::svymean(x = formula(paste0('~',dependent.var)),
                                    design = srvyr_design, na.rm=T) %>% confint(level = confidence_level)
      ...
      error = function(e) {
        ...
        return(hypegrammaR:::datasanitation_return_empty_table(dependent.var = dependent.var,independent.var = independent.var,message = e$message))
      }
    )
  }
...
percent_with_confints_select_multiple <- function(dependent.var,
  ...
  sapply(dependent.var.sm.cols, function(x){
    ...
    if(!sanitised$success){
      warning(sanitised$message)
      return(hypegrammaR:::datasanitation_return_empty_table(data = design$variables, dependent.var.check, message =sanitised$message))
      }
    design<-sanitised$design
    }
    )
...
mean_with_confints <- function(dependent.var,
  ...
  if(!sanitised$success){
    warning(sanitised$message)
    return(hypegrammaR:::datasanitation_return_empty_table(design$variables, dependent.var, message = sanitised$message))}
...
mean_with_confints_groups <- function(dependent.var,
  ...
  if(!sanitised$success){
    warning(sanitised$message)
    return(hypegrammaR:::datasanitation_return_empty_table_NA(design$variables, dependent.var, independent.var, message = sanitised$message))
  }
