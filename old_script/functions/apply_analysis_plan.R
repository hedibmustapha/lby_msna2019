...
from_analysisplan_map_to_output <- function(data,
           ...
           labeled = FALSE,
           verbose = TRUE,
           strata.variable.name=NULL,
           population.variable.name=NULL) {
...
#       }
      ...
      case <- map_to_case(
        ...
        dependent.var.type =
          if (hypegrammaR:::value_is_empty(x["dependent.var.type"]))
            NULL
        else {
          x["dependent.var.type"]
        },
        independent.var.type =
          if (hypegrammaR:::value_is_empty(x["independent.var.type"]))
            NULL
        else {
...
analysisplan_clean<-function(analysisplan){
  analysisplan<-lapply(analysisplan,function(x){
    x[hypegrammaR:::value_is_empty(x)]<-NA
    x
  }) %>% as.data.frame(stringsAsFactors=F)
  analysisplan<-hypegrammaR:::remove_empty_rows(analysisplan)
analysisplan
  }
